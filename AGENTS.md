...
- **[release.yml](.github/workflows/release.yml)** — named **Publish**, not "Release", so it is not confused with "Release Please" in the Actions list. On push of a `v*` tag it builds, signs, notarizes, uploads the DMG + CLI binary onto the Release that release-please already created, and syncs the Homebrew tap. It never creates the Release itself.
