...
public enum DiagnosticsEngine {
    ...
    public static func diagnoseCharging(power: PowerSnapshot?,
                                        adapterMaxWatts: Double?) -> ChargingDiagnostics {
        guard let power else {
            return ChargingDiagnostics(verdict: .unknown, summary: "暂无电源数据")
        }
        guard power.externalConnected else {
            return ChargingDiagnostics(verdict: .notConnected, summary: "未接通电源，使用电池")
        }
        guard power.isCharging else {
            return ChargingDiagnostics(
                verdict: .paused,
                summary: "已接通电源，未在充电",
                detail: "常见于优化充电暂停或电池保温（电量接近充满时系统主动放缓）"
            )
        }

        guard let contractWatts = power.pdContract?.watts, contractWatts > 0 else {
            return ChargingDiagnostics(verdict: .unknown, summary: "正在充电（合同读数缺失）")
        }
        ...
        if let adapterMaxWatts, contractWatts < adapterMaxWatts * 0.95 {
            return ChargingDiagnostics(
                ...
                summary: String(format: "PD 合同 %.0fW 低于适配器可用档 %.0fW", contractWatts, adapterMaxWatts),
                detail: "线缆可能限制了协商档位（未标 5A 的线无法请求大电流档）；电池状态也可能影响，仅供参考"
            )
        }
        ...
        if realWatts >= contractWatts * 0.9 {
            return ChargingDiagnostics(
                verdict: .adapterMax,
                summary: "充电器正按合同满档输出",
                detail: String(format: "当前 %.1fW ≈ PD 合同 %.0fW", realWatts, contractWatts)
            )
        }

        return ChargingDiagnostics(
            verdict: .machineDrawingLess,
            summary: String(format: "实际抽取 %.1fW，低于合同 %.0fW", realWatts, contractWatts),
            detail: "电池接近充满或系统在限流，属正常现象"
        )
    }
    ...
    /// 无任何可说内容时返回 nil。
    public static func portHeadline(port: USBCPortSnapshot?, power: PowerSnapshot?) -> String? {
        ...
        if power?.isCharging == true, let watts = power?.watts, watts > 0 {
            parts.append(String(format: "⚡%.1fW", watts))
        } else if let winning = port.powerSource?.winning {
            parts.append("合同 \(Int(winning.watts.rounded()))W")
        }

        if let eMarker = port.eMarker {
            if let description = eMarker.productTypeDescription {
                parts.append(eMarkerDescription(description))
            }
    ...
    /// Apple 的产品类型描述是英文（"Passive Cable"），展示层统一成中文。
    public static func eMarkerDescription(_ raw: String) -> String {
        switch raw.lowercased() {
        case "passive cable": return "被动线缆"
        case "active cable": return "主动线缆"
        case let v where v.contains("epr"): return "EPR 线缆"
        default: return raw
        }
    }
}
