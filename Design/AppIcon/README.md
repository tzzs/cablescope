...
10. **接入 Xcode 工程（已验证，2026-09-14）**：`CableScope.icon` 已作为源条目写进 `project.yml`（`CableScopeApp` target 的 sources 里，与 `AppIcon.xcassets` 并列）。`xcodegen generate && xcodebuild` 实测：actool 把 `.icon` 与 appiconset 合并编译进同一 `Assets.car`（经典 10 槽位 + Liquid Glass 分层渲染 + 外观 PackedAsset），**无警告**，`CFBundleIconName=AppIcon` 不变——macOS 26 优先渲染分层玻璃，旧系统自动回退经典位图，两路图标共存无需手工切换。`bundle_app.sh`（SwiftPM/DMG 路径）不走 actool，仍用 `.icns`，不受影响。
